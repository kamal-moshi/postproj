...
import { EPProject } from "../models/EpisodeModel.mjs";
...
export const postProjectEpisode = async (req, res) => {
    try {
        ...
        const postEp = await EPProject.create({ title, description, project: projectId });

        return res.status(201).json({ status: true, message: "successfully created", data: postEp });
    } catch (error) {
...
export const updateEd = async (req, res) => {
    try {
        ...
        const updateEdProject = await EPProject.findByIdAndUpdate(EPprojectId, data, { new: true });
